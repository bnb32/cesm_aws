...
if args.years:
    ...
    os.system(f'rm -f ./timing.log')
    for n in ntasks:
        cmd = f'python run_cesm.py -year 0 -ntasks {n} -timing -run_all'
        start_time = time.time()
        os.system(cmd)
        end_time = time.time()
        logger.info(f'Time elapsed: {end_time-start_time}')
        os.system(f'echo "elapsed: {end_time-start_time}, ntasks: {n}" >> timing.log')
